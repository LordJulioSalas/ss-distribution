    ...
    <main className="min-h-screen pt-32 px-4 pb-20">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative rounded-2xl overflow-hidden mb-16 h-64">
          <ImageDistortion src="/images/format_dji0015edit_5cde377a998c1.webp" alt="Industrial operations" className="h-64" />
          <div className="absolute inset-0 flex flex-col items-center justify-center" style={{ background: 'rgba(10,10,15,0.65)' }}>
            <h1 className="text-5xl font-bold gradient-text mb-3">
              <RevealText text={t.home.industriesTitle} />
            </h1>
            <motion.p 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {t.home.industries.map((ind, i) => (
            <TiltCard key={i} className="glass-effect p-8 rounded-xl hover:border-primary transition-all">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-12">
          <ImageDistortion src="/images/barges-harbour-curoil-sole-supplier-fuel-oils-curaçao-bonaire-your-one-stop-location-bunker-fuels-as-43882167.webp" alt="Caribbean operations" className="h-64" />
        </motion.div>
      </div>
    </main>
  )
